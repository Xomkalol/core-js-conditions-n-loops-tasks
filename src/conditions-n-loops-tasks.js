...
function convertToRomanNumerals(num) {
  ...
  for (let i = 0; i < romanNumbers.length; i += 1) {
    ...
    switch (digit) {
      case 9:
        result = romanNumbers[i][0] + romanNumbers[i + 1][0] + result;
        break;
      case 4:
        result = romanNumbers[i][0] + romanNumbers[i][1] + result;
        break;
      default:
        if (digit >= 5) {
          result =
            romanNumbers[i][1] +
            repeatSymbol(romanNumbers[i][0], digit - 5) +
            result;
        } else {
          result = repeatSymbol(romanNumbers[i][0], digit) + result;
        }
    }
  }
  return result;
...
function isPalindrome(str) {
  ...
  for (let i = str.length - 1; i >= Math.ceil(str.length / 2); i -= 1) {
...
function shuffleChar(str, iterations) {
  ...
  let currentStr = str;
  let period = 0;
  let iterationQuanity = iterations;
  do {
    currentStr = performIteration(currentStr);
    period += 1;
  } while (currentStr !== str);
  iterationQuanity %= period;
  currentStr = str;
  for (let i = 0; i < iterationQuanity; i += 1) {
    currentStr = performIteration(currentStr);
  }
  return currentStr;
}
